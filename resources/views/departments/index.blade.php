...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (window.$ && $.fn.DataTable) {
            if ($.fn.DataTable.isDataTable('#dt_PageDepartments')) {
                $('#dt_PageDepartments').DataTable().clear().destroy();
            }

            $('#dt_PageDepartments').DataTable({
                pageLength: 10,
                lengthChange: true,
                ordering: true,
                searching: true,
                ...
                initComplete: function() {
                    $('#dt_PageDepartments_filter').appendTo('#dt_PageDepartments_Search');
                }
            });
        }
        ...
        <div class="card card-action action-border-primary">
            <div class="card-body d-flex justify-content-between align-items-center">
                <div class="clearfix ps-2">
                    ...
                    <span class="text-primary">Total Departments</span>
                </div>
        ...
        <div class="card overflow-hidden">
            <div class="card-header d-flex gap-3 flex-wrap align-items-center justify-content-between border-0 pb-0">
                ...
                <div class="d-flex gap-3 flex-wrap">
                    <div id="dt_PageDepartments_Search"></div>
                </div>
            </div>

            <div class="card-body p-2">
                <table id="dt_PageDepartments" class="table display table-row-rounded">
                    <thead class="table-light">
                        <tr>
                            <th class="minw-100px">ID</th>
                            <th class="minw-220px">Name</th>
                            <th class="minw-260px">Description</th>
                            <th class="minw-140px">Code</th>
                            <th class="minw-150px">Created</th>
                            <th class="minw-150px">Updated</th>
                    ...
                    <tbody>
                        ...
                        $code = $d->code ?? \Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($d->name ?? '', 0, 3));
                        ...
                        <tr>
                            ...
                            <td><span class="text-success">{{ $d->name ?? '-' }}</span></td>
                            <td>{{ $notes ?: '—' }}</td>
                            <td>{{ $code }}</td>
                            <td>{{ $created }}</td>
                            <td>{{ $updated }}</td>
            ...
            <div class="modal-body">
                <form method="POST" action="{{ route('departments.store') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="form-control" rows="3" placeholder="Enter department description">{{ old('notes') }}</textarea>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Department Code (optional)</label>
                        <input type="text" name="code" class="form-control" placeholder="Enter department code" value="{{ old('code') }}">
                    </div>

...
<div class="row">
    ...
    @php
    // ✅ Full Name (NEW DB: first_name + surname)
    $fullName = trim(($employee->first_name ?? '') . ' ' . ($employee->surname ?? ''));

    // ✅ Status (if date_of_exit exists)
    ...
    // ✅ Photo from employee_documents (active "Profile photo")
    $activeDocs = $employee->documents?->where('is_active', 1);
    $photoDoc = $activeDocs?->firstWhere('remarks', 'Profile photo');
    $photoUrl = $photoDoc
    ...
    // ✅ Bank last4
    $bankLast4 = $employee->primaryBankAccount?->account_last4;
    @endphp
            ...
            <div class="card-body p-2 pt-0">
                ...
                <div class="p-3 bg-light rounded">
                    ...
                    <div class="d-grid gap-2">
                        ...
                        {{-- ✅ Bank last 4 --}}
                        <span class="d-block text-dark">
                            <i class="fi fi-rr-bank me-2 text-primary"></i>
                            {{ $bankLast4 ? ('****' . $bankLast4) : '—' }}
                        </span>
                    </div>
                </div>
